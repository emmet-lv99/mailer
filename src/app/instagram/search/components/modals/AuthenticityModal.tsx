            ...
            <div className="space-y-4 w-full px-1 overflow-y-auto max-h-[80%] hide-scrollbar">
                ...
                <div className="text-left">
                    <div className="flex justify-between text-xs mb-1">
                        <span className="font-semibold text-gray-700 flex items-center gap-1">
                            ✅ 댓글 비율 ({authDetails?.commentRatio?.toFixed(2)}%)
                        </span>
                        <span className="font-bold text-green-700">+{authDetails?.commentScore}점 ({Math.round((authDetails?.commentScore / 20) * 100)}%)</span>
                    </div>
                    {(() => {
                ...
                <div className="text-left">
                    <div className="flex justify-between text-xs mb-1 items-center">
                        <span className={`font-semibold flex items-center gap-1 ${authDetails?.viewScore !== null ? 'text-gray-700' : 'text-gray-400'}`}>
                            {authDetails?.viewScore !== null ? '✅' : '⚪'} 조회수 비율 {authDetails?.viewScore !== null ? `(${authDetails.viewRatio.toFixed(3)})` : ''}
                        </span>
                        {authDetails?.viewScore !== null ? (
                            ...
                            <span className="text-gray-400 font-medium flex items-center gap-1 group relative cursor-help">
                                측정 불가 (릴스 없음)
                                <span className="text-[10px] bg-gray-100 rounded-full w-3.5 h-3.5 flex items-center justify-center">?</span>
                ...
                <div className="flex justify-between text-xs border-b border-gray-100 pb-2 items-center">
                    ...
                    {authDetails?.consistencyScore !== null ? (
                        <span className="font-bold text-green-700">+{authDetails.consistencyScore}점</span>
                    ) : (
                        <span className="text-gray-400 font-medium">측정 불가 (릴스 없음)</span>
                    )}
                </div>
