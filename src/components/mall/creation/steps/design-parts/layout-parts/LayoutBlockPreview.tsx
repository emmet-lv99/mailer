...
export function LayoutBlockPreview({ block, borderRadius, onRemove }: LayoutBlockPreviewProps) {
    ...
    if (block.category === 'video-product') {
        return (
            <div className={cn(
                baseClasses,
                "p-3 border border-amber-500/20",
                "bg-amber-500/10 hover:ring-2 hover:ring-amber-500 hover:ring-offset-2 hover:ring-offset-slate-900"
            )} style={{ borderRadius: isFullWidth ? '0px' : borderRadius }}>
                <div className="flex justify-between items-center mb-2">
                    <span className="text-[10px] text-amber-400 font-bold uppercase tracking-widest">{getName(videoProductLayouts, block.type)}</span>
                    <RemoveButton onClick={handleRemove} />
                </div>
                {block.type === 'full-video' || block.type === 'full-image' ? (
                    <div className="w-full h-32 bg-amber-500/20 rounded-lg flex flex-col items-center justify-center gap-2 border border-amber-500/10 relative overflow-hidden">
                        <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center">
                            <div className="w-0 h-0 border-t-[5px] border-t-transparent border-l-[8px] border-l-white border-b-[5px] border-b-transparent ml-1" />
                        </div>
                        <span className="text-[9px] text-amber-200 font-medium uppercase tracking-tighter">Cinematic Experience</span>
                    </div>
                ) : (
                    <div className="flex gap-3 h-32">
                        <div className="w-3/5 bg-amber-500/20 rounded-lg flex items-center justify-center border border-amber-500/10 relative">
                             <div className="w-6 h-6 rounded-full bg-white/20 flex items-center justify-center">
                                <div className="w-0 h-0 border-t-[4px] border-t-transparent border-l-[6px] border-l-white border-b-[4px] border-b-transparent ml-0.5" />
                             </div>
                        </div>
                        <div className="flex-1 space-y-2 py-2">
                             <div className="h-2 w-full bg-amber-500/30 rounded-full" />
                             <div className="h-2 w-2/3 bg-amber-500/20 rounded-full" />
                             <div className="mt-auto pt-2 space-y-1">
                                <div className="h-6 w-full bg-amber-500/40 rounded-md" />
                                <div className="h-6 w-full bg-amber-500/20 rounded-md" />
                             </div>
                        </div>
                    </div>
                )}
